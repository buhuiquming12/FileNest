...
- **各文件夹占用排行**：一次递归扫描后，按直接子文件夹展示大小、占比、文件数和子文件夹数，并列出大文件供分析。`扫描`只收集事实，不调用 AI。
- **AI 清理建议（独立功能）**：基于已完成的扫描，标注“可删除 / 建议检查 / 建议保留”、理由、置信度和来源；只提供建议，不直接删除。
...
| AI 服务超时/不可用 | URL API 超时/异常自动降级为本地建议；扫描结果不受影响 |
| AI 清理建议判断错误 | 建议页只读，不提供直接删除按钮；同时显示置信度、来源与风险理由 |
...
src/test/java/com/filenest          # 10 个测试类，25 个用例
```
...
### 接入 URL AI API
主窗口可直接填写 **AI API 地址、API Key（可选）和模型名**。点击 **“获取模型”** 会根据 Chat Completions URL 自动请求 OpenAI 兼容的 `/models`（同时兼容 Ollama `/api/tags`），并把返回模型放入可编辑下拉框。点击“应用 AI API”后，整理建议和清理建议分别调用模型；扫描本身不会调用 AI。

也可使用环境变量预填客户端：
- `FILENEST_LLM_ENDPOINT` —— 完整 HTTP 端点，例如 `https://example.com/v1/chat/completions`
- `FILENEST_LLM_API_KEY` —— API Key；本地无鉴权 API 可留空
- `FILENEST_LLM_MODEL` —— 请求中的模型名

模型必须返回 `suggestions` JSON 数组。客户端会校验文件名、目标相对路径和置信度；AI 建议始终默认不勾选且必须人工确认。API 超时或异常时自动降级到本地启发式 AI。将地址留空并应用即可切回本地 AI。
...
36 个 JUnit 5 用例，覆盖全类型文件扫描、隐藏文件保护、分类与精确去重、自动整理安全门、路径越界/符号链接阻断、冲突不覆盖、撤销回放、AI 超时降级、URL 模型获取和日志持久化。
