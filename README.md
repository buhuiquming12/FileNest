...
选一个杂乱的文件夹 → 工具用「规则」+「AI 建议」两路信息生成一份整理计划 → 你在表格里勾选确认 → 一键执行 → 随时可一键撤销。
...
| AI 服务超时/不可用 | 超时装饰器自动降级为「无 AI」，规则计划照常产出，系统不瘫痪 |
...
│   │   └── LlmAiAdvisor.java        # 大模型接入骨架（默认不启用）
│   ├── executor/
...
└── ui/
    ├── MainView.java                # 主窗口：目录选择/计划表格/执行/撤销
    └── ActionRow.java               # 表格行视图模型（含勾选状态）

src/test/java/com/filenest          # 7 个测试类，20 个用例
```
...
### 接入真实大模型
`LlmAiAdvisor` 已预留接入点，从环境变量读取配置：
- `FILENEST_LLM_ENDPOINT` —— 模型 HTTP 端点
- `FILENEST_LLM_API_KEY` —— API Key

只需实现其中的 `callModel(String prompt)`（用 `java.net.http.HttpClient` 发请求）与 `parseResponse(...)`，即可替换默认的本地启发式——**UI、执行、撤销逻辑完全不用改**，这是开闭原则最直接的收益。未配置时它自动表现为「无建议」，安全降级。
...
- 工具**只扫描所选目录的顶层常规文件**，不递归子目录、不动隐藏文件——「整理」而非「托管」。
...
20 个 JUnit 5 用例，覆盖安全关键路径：分类、精确去重、执行器（冲突不覆盖 / 撤销回放）、AI 启发式、超时降级、日志持久化、以及端到端服务流程。
